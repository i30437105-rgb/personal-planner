      ...
      <div style={{ position: 'relative', width: '100%', paddingBottom: '140%', transformStyle: 'preserve-3d', transition: 'transform 0.6s ease', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, borderRadius: '16px', overflow: 'hidden', backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)', background: dream.coverImage ? `url(${dream.coverImage}) center/cover` : `linear-gradient(135deg, ${COLORS.bgCard} 0%, ${COLORS.bg} 100%)`, border: `1px solid ${COLORS.border}`, boxShadow: `0 4px 20px rgba(0,0,0,0.4)` }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 16px', zIndex: 1 }}>
            ...
            <p style={{ fontSize: '12px', color: COLORS.textMuted, lineHeight: '1.4', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{((dream.description || dream.prayerText || '').length > 200 ? (dream.description || dream.prayerText || '').slice(0, 200) + '...' : (dream.description || dream.prayerText || ''))}</p>
          </div>
        </div>
      </div>
      {isFocused && !isFlipped && (
...
const DreamDetail = ({ dream, sphere, onClose, onEdit, onToggleFocus, onSetLeading, onArchive, onAchieve, isFocused, isLeading }) => {
  ...
  const fullText = dream.description || dream.prayerText || 'Без описания';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(20px)', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 100, padding: '20px', overflowY: 'auto' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: '500px', display: 'flex', flexDirection: 'column', gap: '16px', paddingTop: '40px', paddingBottom: '40px' }}>
        <div style={{ position: 'relative', width: '100%', borderRadius: '20px', overflow: 'hidden', background: dream.coverImage ? `url(${dream.coverImage}) center/cover` : `linear-gradient(135deg, ${COLORS.bgCard} 0%, ${COLORS.bg} 100%)`, border: `2px solid ${isLeading ? COLORS.gold : COLORS.border}`, boxShadow: `0 20px 60px rgba(0,0,0,0.5)` }}>
          <div style={{ position: 'relative', width: '100%', paddingBottom: '60%' }}>
            <div style={{ position: 'absolute', inset: 0, background: dream.coverImage ? 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.2) 100%)' : 'transparent' }} />
            {isFocused && (
          ...
          <div style={{ padding: '20px', background: COLORS.bgCard }}>
            ...
            <h2 style={{ fontSize: '22px', fontWeight: '600', color: COLORS.text, marginBottom: '12px', fontFamily: 'Georgia, serif' }}>{dream.title}</h2>
            <div style={{ maxHeight: '200px', overflowY: 'auto', marginBottom: '16px', paddingRight: '8px' }}>
              <p style={{ fontSize: '14px', color: COLORS.textMuted, lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>{fullText}</p>
            </div>
